...
export class AssignDiscordGuild {
  ...
  async execute(teacherId: number, classId: number, input: SelectClassDiscordGuildInput) {
    const userGuild = await this.classroomDiscordWriter.findDiscordUserGuildById(
      teacherId,
      input.guild_id,
    );
    ...
    const existingByGuild = await this.classroomDiscordWriter.findDiscordGuildByDiscordGuildId(
      teacherId,
    ...
    if (existing) {
      ...
      return this.classroomDiscordWriter.saveClassDiscordBinding(existing);
    }

    return this.classroomDiscordWriter.saveClassDiscordBinding(
      this.classroomDiscordWriter.createClassDiscordBinding({

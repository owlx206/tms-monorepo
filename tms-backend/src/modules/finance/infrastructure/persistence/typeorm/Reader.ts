...
export class TypeOrmTransactionReader {
  ...
  private validateDateRange(filters: { from?: Date; to?: Date }) {
    if (filters.from && filters.to && filters.from > filters.to) {
      throw new HttpError('from must be earlier than or equal to to', 400);
    }
  }
  ...
  async listStudentBalances(
    ...
    const transactionTotals = await this.manager.getRepository(Transaction)
      .createQueryBuilder('transaction')
    ...
    const feeTotals = await this.manager.getRepository(FeeRecord)
      .createQueryBuilder('fee_record')

...
import { GetBotInviteLinkUseCase } from './GetBotInviteLinkUseCase.js';
...
export class GetDiscordSetupStatusUseCase {
  constructor(
  ...
  async execute(teacherId: number) {
    const [
      ...
      this.messaging.countTeacherDiscordServerCaches(teacherId),
    ]);
